...
function baseHeaders() {
  // Imita lo más posible el request que hace el frontend real de smiles.com.ar:
  // el WAF de Smiles devuelve 406 a requests que no parecen del navegador.
  return {
    'x-api-key': getApiKey(),
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (!res.ok) {
        ...
        throw new HttpError(res.status, `Smiles respondió HTTP ${res.status}${snippet ? ` — ${snippet}` : ''}`);
      }
...
async function debugSearch(q) {
  ...
  return {
    ...
    sentHeaders: { ...sent, 'x-api-key': `${sent['x-api-key'].slice(0, 4)}…${sent['x-api-key'].slice(-4)}` },
  };
}
...
module.exports = { searchDay, getBoardingTax, emissionUrl, debugSearch };
