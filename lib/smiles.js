...
async function engineFetch(url) {
  if (activeEngine === 'http') {
    const res = await fetch(url, { headers: httpHeaders(), signal: AbortSignal.timeout(30000) });
    ...
    try {
      ...
      if (status === 406) {
        throw new HttpError(
          406,
          activeEngine === 'browser'
            ? 'El WAF de Smiles bloquea también al navegador desde esta IP (posible bloqueo de IPs de datacenter). Probá correr la app local: npm start.'
...
async function debugSearch(q, engine) {
  ...
  const res = await fetch(url, { headers: sent, signal: AbortSignal.timeout(30000) });
  const body = await res.text().catch(() => '');
  return {
    url,
    engine: 'http',
...
module.exports = { searchDay, getBoardingTax, emissionUrl, debugSearch, currentEngine };
